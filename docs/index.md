...
???+ info "層級三：人工覆蓋規則之作用機制 (3.1 ～ 3.2)"
    這兩條規則是**系統對人工歷史審核結果的承接機制**：

    * **3.1 套用歷史品牌字串規則（全站共用）**：
      當某個品牌字串先前已經由人工或 Temp 在審核表中確認過（例如將「綠鐘」指定為 `GREEN BELL`），該決策會永久寫入規則庫。日後全站 22 個品類若再次出現該字串，系統不再重新計算或猜測，**直接以此路徑套用既有決策**（信心度 100 分）。
      *(註：專案初次運行前此處為 0 筆，隨著審核回灌而逐步增加)*
    * **3.2 套用單一品號覆蓋規則（單品專屬）**：
      僅用於極少數「字串不能改，但某個特定品號需單獨例外指定」的特殊情況。
