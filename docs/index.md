...
> **版本**：v1.6　｜　**適用範圍**：momo 全站商品（545,572 品號）　｜　**最後更新**：2026/09/22
...
### 1.1 業務痛點與專案目標
電商競網（momo）商品中，品牌欄位填寫格式混亂，常見「中英並列」、「母子品牌重疊」、「型號規格混入品牌欄」、「空白未填」或「副廠周邊配件借用原廠名稱」等現象。

本專案之核心目標在於建立一套**可重複執行tagging方法**，將競網商品全數標準化對應至既有之正式品牌庫，產出一致、乾淨的商品品牌主檔。

### 1.2 原則
為確保跨品類、跨批次之資料標準完全一致，系統全流程嚴格貫徹以下三原則：

1. **先確認品牌實體，產品線不當作獨立品牌**
   - iPhone 是 Apple 產品線，必須對應帶有 Apple 的正確品牌實體；同實體 ID 再依 adg 選最高。
   - 本次品牌庫為 `Apple [1189223]`（adg 36,232,902）優先於 `iPhone Apple [3886708]`（551,234）。Apple House、蘋果西打等不同品牌不參與比較。
   - 只有明確列出的母子品牌關係（Kanebo / KATE、ASUS / ROG）可使用子品牌優先；不同品牌不因出現位置較後就自動勝出。
   - iPhone 保護殼等配件須確認實際製造商，不能只因相容 iPhone 就歸入 Apple。
...
3. **相容周邊與副廠之製造商精準識別**
   - **無獨立品牌之純相容配件**：手機保護貼、通用轉接線、白牌副廠支架等，本體並非原廠所製。系統一律將純相容描述（如「適用 iPhone」、「Switch 副廠」）收斂為**無品牌（No brand）**，避免污染原廠品牌庫銷售統計。
   - **具獨立第三方製造商品牌之周邊**：例如 `【ipega】副廠 Switch 運動指套`、`【犀牛盾】iPhone 15 防摔保護殼`、`【良值】Switch 副廠手把專用腕帶`。系統精準識別其第三方製造商身份，**正確歸入該品牌庫（Ipega、犀牛盾）或建議新增（良值）**，絕不因內文包含「副廠」或相容主機名稱而誤判為無品牌。
...
???+ info "軌道 A：品牌欄有填之決策路徑 (1.1 ～ 1.7)"
    ...
    | **1.3** | 品牌庫完全相符（含子品牌優先） | <span class="badge badge-pool">TYPE_POOL</span> | 96 | 「BURBERRY」完全相符；「Apple iPhone」對應 Apple 同實體最高 adg ID |
...
| `10225061` | Game Kingdom | *(空白)* | 【ipega】副廠 Switch 運動指套健身配件 | **Ipega** | `1146606` | <span class="badge badge-pool">TYPE_POOL</span> | 2.2 標題【】提取 | <span class="badge badge-score">86</span> |
...
> **抽樣解讀**：
> - 範例 1、2 呈現品牌欄有填時，標準品牌之直接命中與重複品牌自動取大機制。
> - 範例 3、4 呈現品牌欄空白時，系統自動從標題括號提取品牌：
>   - 範例 3 提取並命中庫內品牌 `AMANO`。
>   - **範例 4（關鍵機制）**：商品雖標示「副廠 Switch 配件」，但標題開頭有明確獨立配件品牌 `【ipega】`。系統精準識別出 `Ipega` 為配件製造商，正確歸入 `Ipega`（1146606），**絕不因包含「副廠」而誤殺品牌**！
> - 範例 5、6 呈現具備商業品牌特徵（良值、RedMoon）但品牌庫暫無之項目，自動歸入建議新增名單（TYPE_NEW）。
> - 範例 7 呈現無獨立品牌之純相容配件（MacBook 磁吸防窺片），精準識別相容關鍵字並收斂為無品牌（No brand），避免污染原廠 Apple 品牌數據。
...
### 步驟一：一鍵打包產出全站待審總表（含真實商品品名與網址）
- **執行指令**：`py run.py bundle-export`
- **產出檔案**：`review/momo/_momo_全站審核總表_Temp用.xlsx`
- **審核輔助設計（極大化人工作業效率）**：
  1. **跨品類去重收斂**：自動跨 22 個品類，將相同之待審字串去重合併為單一工作表，審核一次即可向全站廣播生效。
  2. **內嵌真實商品品名與網址**：每一筆品牌字串均附帶**「範例商品名稱」**與**「範例商品網址（可直接點擊開啟 momo 商品頁）」**，審核人員若有疑義，點擊連結即可直接檢視真實商品圖片與描述，大幅降低判斷失誤。
  3. **決策動線與視窗凍結**：由左至右依序呈現「品牌欄 ➔ 範例品名與網址 ➔ 系統建議與候選 ➔ 【人工】判斷黃色填寫區」，並凍結標題列與前 6 欄，確保對照時舒適順手。
...
- **產品線正規化與已確認子品牌關係**：
  iPhone 對應 Apple；只有明確列出的母子品牌關係可自動採用子品牌，其餘多品牌歧義維持低信心待審。
- **跨品類部分命中懲罰**：
...
## 7. 常見問答
**Q1：副廠配件或相容周邊（如 iPhone 保護貼、Switch 副廠手把），會不會被誤判為原廠品牌？若配件本身有獨立品牌（如 ipega、犀牛盾）又該如何處理？**
> **說明**：
> 1. **純相容或白牌配件（收斂為 No brand）**：凡商品無獨立第三方品牌，且標題僅提及相容主機與配件詞（如「iPhone 15 鋼化膜」、「適用 Switch 散熱支架」），或標題假借原廠名義但宣告副廠（如「【Apple】副廠充電線」），系統一律判定為相容周邊，**歸入無品牌（No brand，路徑 2.1）**，絕不誤算入原廠 Apple 或 Nintendo 銷售數據。
> 2. **具獨立製造商品牌之配件（正確歸入第三方品牌）**：若周邊配件本身具備合法獨立品牌（如「【ipega】副廠 Switch 手把運動指套」、「【犀牛盾】iPhone 15 防摔殼」、「【良值】Switch 配件」），系統會精準識別其獨立品牌身份，**正確歸入該品牌庫（Ipega、犀牛盾）或建議新增（良值）**，絕不因出現「副廠/Switch/iPhone」字眼而誤殺品牌！
...
**Q3：Apple 與 iPhone 同時出現如何判斷？**
> iPhone 是商品／產品線名稱，對應 Apple 同實體登錄中 adg 最高的 ID。此規則不把第三方配件歸入 Apple，也不把名稱帶 Apple 的其他公司視為同一品牌。
...
- 品牌欄有值仍優先使用品牌欄；空白時取第一個非空 `【】` 或 `[]`，排除已辨識的促銷、規格與描述，再按條件掃描完整商品名稱（原本限前 80 字）。
- 配件／相容描述可能提前結束搜尋；No brand 是系統暫判，不能解讀為已查證沒有品牌。
- 新增建議均需人工查證；啟發式文字規則不能證明是註冊商標，也不能保證找完所有別名。
- 全站總表新增處理分類、明確動作、套用範圍、跨類目衝突與各類目建議；衝突禁止直接整組回灌。
- 匯入驗證錯誤會中止並保留原檔；帶規則版本的舊判斷不能覆蓋較新的人工結論。
- 歷史統計表與例子為 2026-09-21 快照；最新數量以本次重跑輸出為準。Temp 分工與驗收見 `docs/temp_handoff.md`。
